...
function Header() {
  const {loading, mobileMenu, setMobileMenu} = useData()
  ...
  return (
    <div>
      <div className="sticky top-0 z-10 flex flex-row items-center justify-between h-14 px-4 md:px-5 bg-white dark:bg-black border-b-[1px] border-b-slate-700/50">
        ...
        <div className="h-5 flex items-center">
        {
          pageName !== "video" && (
            ...
            onClick={mobileMenuHandler}>
              {mobileMenu ? 
                ( <CgClose className='text-dark dark:text-white text-xl' /> 
                  ) : (
                  <SlMenu className='text-dark dark:text-white text-xl'/>
                )
              }
        ...
        <Link to={'/'} className='flex h-5 items-center'>
          <img src={ytLogo} alt="YouTube Logo" className='h-full hidden dark:md:block' />
          <img src={ytLogoMobile} alt="YouTube Logo" className='h-full  md:hidden' />
          <div className='hidden md:block dark:md:hidden text-black w-28 h-full'>
            <svg viewBox="0 0 90 20" width='100%' height='100%' preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg" >
              <g>
        ...
        <div className="group flex items-center">
          <div className="flex h-8 md:h-10 md:ml-10 md:pl-5 border border-[#303030] rounded-l-3xl group-focus-within:border-blue-500 md:group-focus-within:ml-5 md:group-focus-within:pl-0">
            ...
            <IoIosSearch className='text-dark dark:text-white text-xl'/>
            </div>
            <input 
              type="text"
              className='bg-transparent outline-none text-dark dark:text-white pr-5 pl-5 md:pl-0 w-44 md:group-focus-within:pl-0 md:w-64 lg:w-[500px] placeholder:text-black dark:placeholder:text-white/60'
          ...
          <button  className='w-[40px] md:w-[60px] h-8 md:h-10 flex items-center justify-center border border-l-0 border-[#303030] rounded-r-3xl bg-white/0.1'
           onClick={()=>searchQueryHandler('searchButton')} name='searchButton'>
          <IoIosSearch className='text-dark dark:text-white text-xl'/>
          </button>
        </div>
        <div className="flex items-center">
          <div className="hidden md:flex">
            <div className="flex items-center justify-center h-10 w-10 rounded-full cursor-pointer hover:bg-slate-300/50 dark:hover:bg-[#303030]/[0.6]">
              <RiVideoAddLine className='text-dark dark:text-white text-xl'/>
            </div>
            <div className="flex items-center justify-center h-10 w-10 ml-2 rounded-full cursor-pointer hover:bg-slate-300/50  dark:hover:bg-[#303030]/[0.6]">
              <FiBell className='text-dark dark:text-white text-xl'/>
            </div>
          </div>
            <div className="flex h-10 w-10 overflow-hidden rounded-full md:ml-4">
              <img src="https://xsgames.co/randomusers/assets/avatars/female/4.jpg" alt="avatar" />
            </div>
        </div>
      </div>
